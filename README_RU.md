...
Вы можете воспользоваться публичной и бесплатной версией опубликованной на [Vercel](https://torapi.vercel.app/api/provider/list), а также развернуть его самостоятельно на любом облачной платформе используя **serverless**, или локально с помощью **Docker**. 
...
Для работы API не нужен токен доступа и авторизация в трекерах. По мимо этого, при использовании публичной версии не требуется VPN, так как данный сервис выступает в роле шлюза, что позволяет интегрировать его в любой проект, а также использовать RSS без сетевых ограничений.

Реализация простого интерфейса (**frontend**) доступна через расширение Google Chrome 🍿 [Libre Kinopoisk](https://github.com/Lifailon/LibreKinopoisk) для одновременного и быстрого поиска раздач во всех доступных торрент-трекерах (like Jackett).
...
**\*** Регистрация требуется только при скачивании торрент-файла по прямой ссылке. Все раздачи при поиске по **id** (идентификатору) содержат **хэш сумму** и **магнитные ссылки** (уже содержат актуальный список серверов торрент-трекеров), которые позволяют сразу начать скачивание содержимого раздачи или сгенерировать торрент-файл с помощью любого торрент-клиента после загрузки метаданных.
