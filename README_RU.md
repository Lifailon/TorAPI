...
Неофициальный API сервер (**backend**) торрент трекеров RuTracker, Kinozal, RuTor и NoNameClub для быстрого поиска раздач, а также получения torrent-файлов, магнитных ссылок, info hash и подробной информации о раздаче по названию фильма, сериала или идентификатору раздачи указанного провайдера, а также предоставляет новостную RSS ленту для всех провайдеров.
...
Во время тестирования запускается локальный сервер, делается запрос к конечной точке `/api/provider/test`, и завершает свою работу.
...
- 🔎 [LibreKinopoisk Chrome Extension](https://github.com/Lifailon/LibreKinopoisk) - добавляет кнопки на сайт Кинопоиск и предоставляет интерфейс **TorAPI** для быстрого поиска фильмов и сериалов в открытых источниках.
